...
//include_once("../includes/inc_session.php"); //do not check session
session_start(); //for random secure image
...
//$sponsorUsername = $_SESSION["sUserName"]; //default , do not use session here...
if ($_SERVER['REQUEST_METHOD'] == 'GET'){
	$u = isset($_GET['u'])?fValidateInput($_GET['u']): '';
	if ($u != ''){
		$sponsorName 	= fGetName($u, $conn);
	}

	if ($sponsorName == "") {
		if(!isset($_COOKIE["cSpUsername"])) {
			$registerMessage	.= "Wrong sponsorship link";
		}else{
			$sponsorUsername = $_COOKIE["cSpUsername"];
			$sponsorName 	 = fGetName($sponsorUsername, $conn);
		}
	}else{
		$sponsorUsername = $u;
		fSetCookies("cSpUsername", $sponsorUsername, 0);
	}
}
...
if (!empty($_POST)) { 
	...
	$tjVerifyCode = strtotime('now'); // don't use uniqid(); at this part 
	...
	if ($currDate <= $minUmur){
		...
		if ($package != "" && $username != "" && $name != "" && $IDType != "" && $IDN != "" && $sponsorUsername != "" && $password != "" && $codeMobile != "" && $mobile != "" && $email != "" && $country != "" && $state != "" && $city != "" && $address != "" )
		{
			...
			if ($code != $_SESSION["code"]){
				...
				$sql = "SELECT mbrUsername FROM dtMember WHERE mbrUsername='" . $sponsorUsername . "'";
				...
				if ($query->num_rows == 0){
					...
					if ($query->num_rows > 0){
						...
						//send notif to admin
						//fSendToAdmin('REGISTER_FAILED_USER_DUPLICATE', 'regLink.php', 'username duplicate: ' . $username);
					}else {
						if (fInsert("dtTempJoin", $arrData, $conn)){
							...
							if (!fSendNotifToEmail("REGISTER_SUCCESS", $username)){
								//fail sending email
								fSendToAdmin('REGISTER_NEW_MEMBER', 'regLink.php', 'send email failed');
							}
							//fSendToAdmin('REGISTER_NEW_MEMBER', 'regLink.php', 'send success');
							//redirect to success page
							header("Location: regLink.php?u=".$sponsorUsername."&q=info-success");
							die();
						}else{
							//insert fail	
							...
							//send notif to admin
							fSendToAdmin('REGISTER_NEW_MEMBER', 'regLink.php', 'sql: insert failed');
						} // end else
			...
			$registerMessage = "Incomplite Data";
			//fSendToAdmin('REGISTER_NEW_MEMBER', 'regLink.php', 'sql: ' . $registerMessage);
		}
		*/
	}
} //end $_POST
	...
	function getDataSponsor(){
		...
		if ($id != ""){
			$.get("getData.php?q=sponsor&id="+$id, function(data, status){
				...
				if ((data == "wrong sponsor's username")  || (data == "not under your genealogy tree")){
	...
	*/
	...
	$(document).ready(function(e) {
		...
		*/
		...
		/*$("#submit").click( function(){
			if ($("#agree:checked").val() == ""){
				alert ("ok");	
			}else alert ($("#agree:checked").val() + "adfaf");
		});
		*/
			
		/*
		//Check tick agree checkedbox
		$("#submit").click( function(){
			if ($("#agree").is(":checked")){
				//okay	
			}else{
		...
		*/
		...
		$("#submit").click(function(){
			//check package
			if ($.trim($('select[name="package"]').val()) == ''){
				demo.showNotification('top','center', 'info', 'Please pick a <b>Package</b>');
			}
			//check ID Type
...
<body>
    <span id="q"><?php echo (isset($_GET["q"])?$_GET["q"]:""); ?></span>
<div class="card">
    ...
    <div class="card-body card-fix">
        <div class="content">
            ...
            <form action="regLink.php?u=<?php echo $sponsorUsername ?>" method="post">
            	...
            	<div class="row">
                	<div class="col-md-6">
                    	<div class="row">
                            ...
                            <select class="selectpicker" data-size="7" data-style="btn btn-primary" name="package" title "Package">
                                <option disabled selected>Select Package</option>
                                <?php 
									$sql = "SELECT * FROM msPackage ORDER BY pacPrice";
									$query = $conn->query($sql);
									if ($query->num_rows > 0){
										while ($row = $query->fetch_assoc()){
											$selected =  ($package == $row["pacID"])?" selected " : "";
                        ...
                        <div class="row">
                        	...
                        	<div class="form-group col-md-4">
                                <input id="sponsorUsername" name="sponsorUsername" type="hidden"  class="form-control" value="<?php echo $sponsorUsername ?>">
                                <input name="dispalySponsorUsername" type="text"  class="form-control" value="<?php echo $sponsorUsername ?>" disabled>

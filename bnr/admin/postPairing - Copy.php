...
$postingDate	= "2018-04-07"; //date("Y-m-d"); //"2018-04-09"; //
...
if ($queryMember = $conn->query($sql)){
		...
		if ($queryPair = $conn->query($sqlPair)){
			if ($rowPair = $queryPair->fetch_assoc()){
				if ($mbrPos == "R"){ //right
                    ...
                    */
                    $sqlSelect = "SELECT pairLeft, pairRight, mbrUsername FROM dtDailyPairing WHERE mbrUsername='" . $mbrUpline . "' AND DATE(pairDate) ='" . $postingDate . "'";
                    $querySelect = $conn->query($sqlSelect);
                    if ($row = $query->num_rows> 0){
                        //update
                        $sqlUpdate  = "UPDATE dtDailyPairing SET pairRight=pairRight+" . $rowPair['totalPair'];
                        $sqlUpdate .= " WHERE pairUsername='" . $mbrUpline . "' AND pairDate='" . $postingDate . "'";
                        $queryUpdate = $conn->query($sqlUpdate);
                    }else{
                        //insert
                        $table = "dtDailyPairing";
                        $arrData = array(
                                0 => array ("db" => "pairUsername"      , "val" => $mbrUpline),
                                1 => array ("db" => "pairDate"          , "val" => $postingDate),
                                2 => array ("db" => "pairRight"         , "val" => $rowPair['totalPair'])
                                );
                    ...
                    */
                    ...
                    if ($row = $query->num_rows> 0){
                        //update
                        $sqlUpdate  = "UPDATE dtDailyPairing SET pairLeft=pairLeft+" . $rowPair['totalPair'];
                        $sqlUpdate .= " WHERE pairUsername='" . $mbrUpline . "' AND pairDate='" . $postingDate . "'";
                        $queryUpdate = $conn->query($sqlUpdate);
                    }else{
                        //insert
                        ...
                        $arrData = array(
                                0 => array ("db" => "pairUsername"      , "val" => $mbrUpline),
                                1 => array ("db" => "pairDate"          , "val" => $postingDate),
                                2 => array ("db" => "pairLeft"         , "val" => $rowPair['totalPair'])
                                );
                        fInsert($table, $arrData, $conn);
                    }
                    echo $sqlUpdate . "<br>";
    ...
    $sql .= " ( ";
    $sql .= "     SELECT IF((sumDailyTO - sumWK) > pacFlushOut, ( IF (pacFlushOut > ((sumDailyTO - sumWK) - todayPair), (pacFlushOut - ((sumDailyTO - sumWK) - todayPair)) * 10/ 100, 0 ) ), (todayPair * 10/100) ) as dailyTO  FROM ";
    $sql .= "     ( ";
    $sql .= "         SELECT pair.pairUsername, pacFlushOut, IF(sumLeft > sumRight, sumRight, sumLeft) as sumDailyTO, (IFNULL(sumTO, 0) + IFNULL(sumFO, 0)) as sumWk, pair.pairDate, IF (todayPairLeft > todayPairRight, todayPairLeft, todayPairRight) as todayPair";
    $sql .= "         FROM ";
    $sql .= "         ( ";
    $sql .= "             SELECT dailyPairing.pairUsername, SUM(dailyPairing.pairLeft) as sumLeft, SUM(dailyPairing.pairRight) as sumRight, pairDate , today.pairLeft AS todayPairLeft, today.pairRight AS todayPairRight FROM  ";
    $sql .= "               (SELECT * FROM dtDailyPairing ORDER BY DATE(pairDate) DESC) dailyPairing ";
    $sql .= "               LEFT JOIN ";
    $sql .= "               (SELECT pairUsername, pairLeft, pairRight FROM dtDailyPairing WHERE pairDate='" . $postingDate . "') today on dailyPairing.pairUsername=today.pairUsername ";
    $sql .= "             WHERE DATE(pairDate) <= '" . $postingDate . "' ";
    $sql .= "             GROUP BY pairUsername ";
    $sql .= "         ) pair  ";
    $sql .= "         INNER JOIN ";
